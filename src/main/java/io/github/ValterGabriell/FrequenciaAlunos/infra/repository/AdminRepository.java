...
public interface AdminRepository extends JpaRepository<Admin, String> {
    ...
    @Query(value = "SELECT * FROM table_admin WHERE email = :email AND tenant =:tenant", nativeQuery = true)
    Optional<Admin> findByEmail(@Param("email") String email, @Param("tenant") Integer tenant);

    @Query(value = "SELECT * FROM table_admin WHERE cnpj = :cnpj AND tenant =:tenant", nativeQuery = true)
